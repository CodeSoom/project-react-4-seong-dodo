...
import TRANSACTION_LIST from '../../fixtures/transaction-list';
...
describe('accountbook reducer', () => {
  ...
  context('with state', () => {
    ...
    it('listens setNextPage action', () => {
      const nextPage = 1;
      const initialState = {
        nextPage: 0,
      };

      const state = reducer(initialState, setNextPage({ nextPage }));

      expect(state.nextPage).toEqual(1);
    });

    it('listens setAnnualTransaction action', () => {
      const transactionHistoryResponseList = TRANSACTION_LIST;
      const initialState = {
      ...
      const state = reducer(
        initialState,
        setAnnualTransaction({ transactionHistoryResponseList }),
      );

      expect(state.transactionHistoryResponseList).toEqual(TRANSACTION_LIST);
    });

    it('listens setAnnualTransactionTotalCount action', () => {
      const totalCount = 2;
      const initialState = {
        totalCount: 0,
      };

      const state = reducer(initialState, setAnnualTransactionTotalCount({ totalCount }));
